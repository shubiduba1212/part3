...
import CultureSalesApi from './apis/CultureSalesApi';
import CultureDetailApi from './apis/CultureDetailApi';

export default function App() {

  // Api 호출시 상태 저장을 위한 설정
  const [data, setData] = useState(null); // 공연/전시 정보 전체 조회
  const [salesData, setSalesData] = useState(null) // 공연 판매 순위 조회
  const [detailData, setDetailData] = useState(null) // 공연/전시 상세 조회

  useEffect(
    () => {
      CultureApi({ setData }); // App.js의 setData함수를 객체 형태로 CultureApi 컴포넌트에 props로 전달
      CultureSalesApi({setSalesData});
      CultureDetailApi({setDetailData});
    },[]
  );

  useEffect(() => {
    console.log("공연/전시 정보 전체 조회 : " + data); // 데이터가 변경될 때마다 로그를 출력
    console.log("공연 판매 순위" + JSON.stringify(salesData)); // 데이터가 변경될 때마다 로그를 출력
    console.log("공연 / 전시 상세 정보 : " + JSON.stringify(detailData)); // 데이터가 변경될 때마다 로그를 출력
  }, [data, salesData, detailData]);
